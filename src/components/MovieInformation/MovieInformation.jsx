...
function MovieInformation() {
  ...
  if (error) {
    return (
      <Box display="flex" justifyContent="center" alignItems="center">
        <Link to="/"> Hay aksi! Bir şeyler ters gitti. Ana sayfaya geri dön. </Link>
      </Box>
    );
  }
  ...
  return (
    <Grid container className={classes.containerSpaceAround}>
          ...
          <div className={classes.buttonsContainer}>
            <Grid item xs={12} sm={6} className={classes.buttonsContainer}>
              <ButtonGroup variant="outlined" size="small">
                <Button target="_blank" rel="noopener noreferer" href={data?.homepage} endIcon={<Language />}>Web</Button>
            ...
            <Grid item xs={12} sm={6} className={classes.buttonsContainer}>
              <ButtonGroup variant="outlined" size="medium">
                <Button onClick={addToFavorites} endIcon={isMovieFavorited ? <FavoriteBorderOutlined /> : <Favorite />}>
                  {isMovieFavorited ? 'Favori' : 'Favori'}
                </Button>
                <Button onClick={addToWatchlist} endIcon={isMovieWatchlisted ? <Remove /> : <PlusOne />}>
                  {isMovieWatchlisted ? 'Liste' : 'Liste'}
                </Button>
      ...
      <Box marginTop="5rem" width="100%">
        ...
        {recommendations ? <MovieList movies={recommendations} numberOfMovies={12} /> : <Box>Bu film için öneri bulunamadı.</Box>}
      </Box>
